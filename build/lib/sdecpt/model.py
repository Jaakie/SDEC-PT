...
def train(
    ...
    :param idxes: list of random indexes to use to construct the constraint matrix
...
def predict(
    ...
    :return: tuple of prediction, actual and prediction probabilities if return_actual is True otherwise prediction and
    ...
    for batch in data_iterator:
        ...
        hidden_features.append(model.encoder(batch).detach().cpu())
    if return_actual:
        return torch.cat(features).max(1)[1], torch.cat(actual).long(),torch.cat(features),torch.cat(hidden_features)
    else:
        return torch.cat(features).max(1)[1], torch.cat(hidden_features)
